...
function Card({
  ...
  return (
    ...
    >
      <motion.div
        className="absolute w-full h-full top-0 left-0 group transition-all duration-300 ease-in-out overflow-hidden rounded-lg"
        style={{ opacity: isActive ? "1" : "0.7" }}
      >
        <Image
          src={generateImagePath({ src: poster_path })}
          fill
          className="object-cover rounded-lg group-hover:scale-110 transition-all duration-300 ease-in-out group-hover:-rotate-3"
          alt={title}
        />
